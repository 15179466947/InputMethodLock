...
| 功能 | 说明 |
|---|---|
| 英文锁定 | 直读 TSF 激活 profile：激活的是输入法（搜狗/微软拼音）→ 切到纯键盘布局直通英文；IMM32 老应用按输入法类型锁中英状态或关闭 IME |
| 中文锁定 | 直读 TSF 激活 profile：不是中文输入法 → 激活快照中的中文输入法；已是中文输入法时锁中文状态（第三方输入法无视状态写入时模拟 Shift） |
...
| 停用恢复 | 默认自动恢复锁定前的输入法（快照）；也可选"不切换"或指定任意输入法 |
...
UnlockLayout=auto         # 停用锁定时：auto=恢复锁定前快照 | none=不切换 | hkl:XXXXXXXX=指定
